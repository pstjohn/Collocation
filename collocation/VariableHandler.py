    ...
    def __getitem__(self, indices):
        ...
        for i, index in enumerate(indices):
            if isinstance(index, slice):
                indices[i] = list(range(*index.indices(self.shape[i])))
        ...
        desiredshape = [len(i) for i in indices if len(i) is not 1]
        if len(desiredshape) < 2:
            desiredshape += [1]*(2 - len(desiredshape))
        elif len(desiredshape) > 2:
        ...
        raveled_index = np.ravel_multi_index(multi_index_tuples, self.shape)
        
        return self._data[raveled_index].reshape(
            (desiredshape[1], desiredshape[0])).T
